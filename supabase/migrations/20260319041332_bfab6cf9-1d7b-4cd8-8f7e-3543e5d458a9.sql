...
SELECT
  ...
  COALESCE(us.completed_visits, 0)        AS completed_visits,
  COALESCE(us.cancel_under_1h, 0)         AS cancel_under_1h,
  COALESCE(us.cancel_under_3h, 0)         AS cancel_under_3h,
  COALESCE(us.no_show_count, 0)           AS no_show_count,
  COALESCE(us.total_cancellations, 0)     AS total_cancellations,
  COALESCE(us.vip_by_count, 0)            AS vip_by_count,
  COALESCE(us.blacklist_by_count, 0)      AS blacklist_by_count,
  COALESCE(us.account_age_days, 0)        AS account_age_days,
  COALESCE(us.status, 'insufficient_data') AS status
FROM public.user_scores us;

GRANT SELECT ON public.user_scores_public TO authenticated;

-- Update calculate_user_score to persist results to user_scores flat columns
...
BEGIN
  ...
  INSERT INTO public.user_scores (
    ...
    unique_partners, top_partner_pct, has_full_name, has_photo, status,
  ...
  VALUES (
    ...
    _unique_executors, _top_pct, _has_full_name, _has_photo, _status,
