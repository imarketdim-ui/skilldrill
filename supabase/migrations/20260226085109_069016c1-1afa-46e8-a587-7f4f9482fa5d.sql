...
ALTER TABLE public.user_scores ADD COLUMN IF NOT EXISTS kyc_verified boolean NOT NULL DEFAULT false;
...
CREATE POLICY "Users view own fraud flags"
  ON public.fraud_flags FOR SELECT
  USING (user_id = auth.uid());

CREATE INDEX IF NOT EXISTS idx_fraud_flags_user_id ON public.fraud_flags(user_id);
CREATE INDEX IF NOT EXISTS idx_fraud_flags_unresolved ON public.fraud_flags(is_resolved) WHERE is_resolved = false;

-- Rewrite calculate_user_score
...
-- Triggers
CREATE OR REPLACE FUNCTION public.recalc_score_on_booking() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$ BEGIN PERFORM public.calculate_user_score(NEW.client_id); RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_recalc_score_on_booking ON public.bookings;
CREATE TRIGGER trg_recalc_score_on_booking AFTER UPDATE OF status ON public.bookings FOR EACH ROW WHEN (NEW.status IN ('completed', 'cancelled')) EXECUTE FUNCTION public.recalc_score_on_booking();

CREATE OR REPLACE FUNCTION public.recalc_score_on_blacklist() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$ BEGIN IF TG_OP = 'DELETE' THEN PERFORM public.calculate_user_score(OLD.blocked_id); RETURN OLD; ELSE PERFORM public.calculate_user_score(NEW.blocked_id); RETURN NEW; END IF; END; $$;
DROP TRIGGER IF EXISTS trg_recalc_score_on_blacklist ON public.blacklists;
CREATE TRIGGER trg_recalc_score_on_blacklist AFTER INSERT OR DELETE ON public.blacklists FOR EACH ROW EXECUTE FUNCTION public.recalc_score_on_blacklist();

CREATE OR REPLACE FUNCTION public.recalc_score_on_rating() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$ BEGIN PERFORM public.calculate_user_score(NEW.rated_id); RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_recalc_score_on_rating ON public.ratings;
CREATE TRIGGER trg_recalc_score_on_rating AFTER INSERT ON public.ratings FOR EACH ROW EXECUTE FUNCTION public.recalc_score_on_rating();

CREATE OR REPLACE FUNCTION public.recalc_score_on_dispute() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$ BEGIN PERFORM public.calculate_user_score(NEW.initiator_id); PERFORM public.calculate_user_score(NEW.respondent_id); RETURN NEW; END; $$;
DROP TRIGGER IF EXISTS trg_recalc_score_on_dispute ON public.disputes;
CREATE TRIGGER trg_recalc_score_on_dispute AFTER UPDATE OF status ON public.disputes FOR EACH ROW WHEN (NEW.status IN ('resolved_for_initiator', 'resolved_for_respondent', 'resolved')) EXECUTE FUNCTION public.recalc_score_on_dispute();

-- Helper function
CREATE OR REPLACE FUNCTION public.check_fraud_indicators(_user_id uuid) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$ DECLARE _flags jsonb; _count integer; BEGIN SELECT COUNT(*) INTO _count FROM fraud_flags WHERE user_id = _user_id AND is_resolved = false; SELECT jsonb_agg(jsonb_build_object('id', id, 'type', flag_type, 'severity', severity, 'description', description, 'created_at', created_at)) INTO _flags FROM fraud_flags WHERE user_id = _user_id AND is_resolved = false ORDER BY created_at DESC; RETURN jsonb_build_object('user_id', _user_id, 'unresolved_count', _count, 'flags', COALESCE(_flags, '[]'::jsonb)); END; $$;
