...
import { Search, User, Dumbbell, Ban, TrendingUp } from 'lucide-react';
...
const FitnessClients = () => {
  ...
  const fetchClients = async () => {
    ...
    const { data: profiles } = await supabase.from('profiles')
      .select('id, first_name, last_name, email, skillspot_id').in('id', clientIds);
    ...
    const list: ClientInfo[] = (profiles || []).map(p => {
      ...
      return {
        ...
        totalWorkouts: cb.length,
        completedWorkouts: cb.filter(b => (b.lessons as any)?.status === 'completed').length,
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground">{search ? 'Клиенты не найдены' : 'У вас пока нет клиентов'}</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="py-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Avatar><AvatarFallback className="bg-primary/10 text-primary">{c.first_name?.[0] || c.email?.[0] || 'C'}</AvatarFallback></Avatar>
                    <div>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="text-center"><p className="font-semibold">{c.totalWorkouts}</p><p className="text-muted-foreground">Тренировок</p></div>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          {selectedClient && (
            <div className="space-y-4">
              ...
              <Card>
                <CardHeader><CardTitle className="text-base">История тренировок</CardTitle></CardHeader>
                <CardContent>
                  {clientWorkouts.length === 0 ? (
                    <p className="text-center py-4 text-muted-foreground">Нет данных</p>
                  ) : (
                    <div className="space-y-2">
                      {clientWorkouts.map(b => (
