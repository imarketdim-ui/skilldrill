...
const FitnessSchedule = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      if (formData.recurrence !== 'none' && formData.recurrence_end) {
        ...
        if (items.length > 0) {
          const { error } = await supabase.from('lessons').insert(items as any);
  ...
  const handleDelete = async (id: string, deleteAll = false) => {
    try {
      if (deleteAll) {
        const w = workouts.find(l => l.id === id);
        if (w?.recurring_pattern_id) {
          await supabase.from('lessons').delete().eq('recurring_pattern_id', w.recurring_pattern_id).eq('status', 'scheduled');
          await supabase.from('recurring_patterns').update({ is_active: false }).eq('id', w.recurring_pattern_id);
        }
      } else {
        ...
        <div className="flex items-center gap-2">
          ...
          <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
            <DialogTrigger asChild>
              <Button size="sm"><Plus className="h-4 w-4 mr-1" /> Тренировка</Button>
            </DialogTrigger>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={formData.title} onChange={e => setFormData(p => ({ ...p, title: e.target.value }))} placeholder="Силовая тренировка" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.lesson_type} onValueChange={v => setFormData(p => ({ ...p, lesson_type: v as any }))}>
                      ...
                      <SelectContent>
                        <SelectItem value="individual">Персональная</SelectItem>
      ...
      {view === 'day' && (
        <div className="space-y-2">
          ...
          {getForDate(currentDate).length === 0 ? (
            <p className="text-center py-12 text-muted-foreground">Нет тренировок на этот день</p>
      ...
      {view === 'month' && (
        <div className="grid grid-cols-7 gap-1">
          ...
          {eachDayOfInterval({ start: startOfWeek(startOfMonth(currentDate), { weekStartsOn: 1 }), end: endOfWeek(endOfMonth(currentDate), { weekStartsOn: 1 }) }).map(day => {
            ...
            const isToday = isSameDay(day, new Date());
            return (
              <div key={day.toISOString()} className={`min-h-[60px] p-1 rounded border text-xs ${!isCurrent ? 'opacity-40' : ''} ${isToday ? 'border-primary bg-primary/5' : 'border-border'}`}>
                <p className="font-medium">{format(day, 'd')}</p>
                {items.length > 0 && <Badge variant="secondary" className="text-[10px] mt-0.5">{items.length}</Badge>}
              </div>
            );
          })}
        </div>
