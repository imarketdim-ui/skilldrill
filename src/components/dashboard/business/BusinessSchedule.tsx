...
const BusinessSchedule = ({ businessId }: Props) => {
  ...
  useEffect(() => { fetchData(); }, [businessId, weekStart]);
  ...
  const fetchData = async () => {
    ...
    const [mastersRes, bookingsRes] = await Promise.all([
      ...
      supabase
        .from('bookings')
        .select('*, service:services!bookings_service_id_fkey(name), client:profiles!bookings_client_id_fkey(first_name, last_name)')
  ...
  const getBookingsForDay = (date: Date, masterId?: string) => {
    return bookings.filter(b => {
      const bDate = parseISO(b.scheduled_at);
      const sameDay = isSameDay(bDate, date);
      if (masterId) return sameDay && b.executor_id === masterId;
      return sameDay;
    });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground">Общее расписание с разбивкой по мастерам</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card><CardContent className="pt-4 pb-3 px-4">
          <p className="text-sm text-muted-foreground">На неделе</p>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground">Добавьте мастеров в команду для отображения расписания</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card key={master.id}>
                ...
                <CardContent>
                  <div className="grid grid-cols-7 gap-1">
                    {days.map(day => {
                      const dayBookings = getBookingsForDay(day, master.id);
                      const isToday = isSameDay(day, today);
                      return (
                        <div key={day.toISOString()} className={`min-h-[80px] rounded-lg border p-1.5 ${isToday ? 'border-primary bg-primary/5' : ''}`}>
                          ...
                          {dayBookings.length === 0 ? (
                            ...
                            <div className="space-y-0.5">
                              {dayBookings.slice(0, 3).map(b => (
                                <div key={b.id} className={`text-[10px] p-1 rounded border ${statusColor(b.status)}`}>
                                  <p className="font-medium truncate">{format(parseISO(b.scheduled_at), 'HH:mm')}</p>
                      ...
                      <div key={b.id} className="flex items-center justify-between p-3 rounded-lg border">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium truncate">{b.service?.name || 'Услуга'}</p>
                            <p className="text-xs text-muted-foreground truncate">
                              {b.client?.first_name} {b.client?.last_name} → {master?.name || 'Мастер'}
                            </p>
                          </div>
                        </div>
                        <Badge variant="outline" className="shrink-0 text-xs">{statusLabel(b.status)}</Badge>
                      </div>
                    );
                  })}
                </div>
