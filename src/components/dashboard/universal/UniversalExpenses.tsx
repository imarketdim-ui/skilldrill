...
const UniversalExpenses = ({ config }: Props) => {
  const { user } = useAuth();
  ...
  const totalByCategory = config.expenseCategories.map(cat => ({ category: cat, total: expenses.filter(e => e.category === cat).reduce((s, e) => s + Number(e.amount), 0) })).filter(c => c.total > 0);
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Select value={form.category} onValueChange={v => setForm(p => ({ ...p, category: v }))}>
                  ...
                  <SelectContent>{config.expenseCategories.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
...
export default UniversalExpenses;
