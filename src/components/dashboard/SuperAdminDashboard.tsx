...
import { 
  ...
  CheckCircle, XCircle, Loader2, TrendingUp, TrendingDown,
...
const SuperAdminDashboard = () => {
  ...
  const assignAdmin = async (userId: string) => {
    try {
      ...
      toast({ title: 'Приглашение отправлено', description: 'Пользователь должен принять или отклонить' });
      loadData();
    } catch (err: any) {
  ...
  const removeAdmin = async (userId: string) => {
    try {
      await supabase.from('user_roles').delete().eq('user_id', userId).eq('role', 'platform_admin');
      toast({ title: 'Администратор снят' });
      loadData();
    } catch (err: any) {
      toast({ title: 'Ошибка', description: err.message, variant: 'destructive' });
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            <p className="text-3xl font-bold">{stats.masters}</p>
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            <p className="text-3xl font-bold">{stats.networks}</p>
            <p className="text-sm text-muted-foreground">Сетей</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="admins">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Найдите пользователя по SkillSpot ID</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex gap-2 mb-4">
                <Input
                  placeholder="Введите SkillSpot ID (например AB1234)"
                  value={searchId}
                ...
                <div className="p-4 rounded-lg border mb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Button onClick={() => assignAdmin(searchResult.id)}>
                      <UserPlus className="h-4 w-4 mr-1" /> Назначить админом
                    </Button>
                  </div>
                </div>
              )}
