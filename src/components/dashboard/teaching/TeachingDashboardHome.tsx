...
import { Calendar, Users, Banknote, AlertTriangle, Clock, CheckCircle, MessageSquare, Plus, GraduationCap, User } from 'lucide-react';
...
const TeachingDashboardHome = () => {
  ...
  const [stats, setStats] = useState({
    todayLessons: 0, todayIndividual: 0, todayGroup: 0,
    totalStudents: 0, studentGrowth: 0,
  ...
  const getActivityText = (event: any) => {
    ...
    switch (event.status) {
      case 'confirmed': return `${name} записался на занятие`;
      case 'completed': return `Занятие с ${name} завершено`;
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            ...
            <Badge variant="secondary">{upcomingLessons.length} занятий</Badge>
          </CardHeader>
          <CardContent>
                  ...
                  return (
                    <div key={lesson.id} className="flex items-center gap-3">
                      <div className="text-center shrink-0 w-16">
                        <p className="text-xs text-muted-foreground">{getLessonDateLabel(lesson.lesson_date)}</p>
